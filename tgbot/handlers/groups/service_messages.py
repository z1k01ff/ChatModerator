...
async def updated_chat_member(
    chat_member_updated: types.ChatMemberUpdated, repo: RequestsRepo
):
    ...
    if chat_member_updated.new_chat_member.status == ChatMemberStatus.KICKED:
        ...
        if chat_member_updated.old_chat_member.status != ChatMemberStatus.ADMINISTRATOR:
            await repo.chat_admins.add_chat_admin(
                chat_member_updated.chat.id, chat_member_updated.from_user.id
            )
            text = (
                f'Пользователь {member_mention} был повышен до статуса Администратора чата с титулом: '
        ...
        new_status = {
            ChatMemberStatus.RESTRICTED: "Ограниченный",
            ChatMemberStatus.MEMBER: "Пользователь",
            ChatMemberStatus.KICKED: "Кикнутый",
            ChatMemberStatus.LEFT: "Вышедший",
        }.get(chat_member_updated.new_chat_member.status, "Пользователь")

        await repo.chat_admins.del_chat_admin(
            chat_member_updated.chat.id, chat_member_updated.from_user.id
        )
        text = f"Администратора {member_mention} понизили до статуса {new_status}"
    else:
        return
