...
async def updated_chat_member(
    ...
    if chat_member_updated.new_chat_member.status == ChatMemberStatus.MEMBER:
        ...
        await repo.rating_users.update_rating_by_user_id(new_member_id, 5)
        ...
        text = (
            f"{member_mention} був видалений з чату користувачем {performer_mention}."
        )
    elif (
        ...
        if chat_member_updated.old_chat_member.status != ChatMemberStatus.ADMINISTRATOR:
            ...
            text = (
                f"{performer_mention} змінив права для адміністратора {member_mention}."
            )
    elif (
